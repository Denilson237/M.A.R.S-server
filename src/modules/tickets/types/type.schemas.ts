...
const TypeReasonsSchema = z.object({
  type: z.string()
...
// Define the schema for bulk create requests
const TypeReasonsbulkCreateSchema = z.object({
  data: z.array(schema).min(1, { message: "At least one bank must be provided." })
});


// Export the schemas
export { schema, bulkCreateSchema , bulkDeleteSchema ,TypeReasonsSchema ,TypeReasonsbulkCreateSchema };
